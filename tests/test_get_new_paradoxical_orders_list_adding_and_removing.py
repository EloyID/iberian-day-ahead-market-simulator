...
class TestGetParadoxicalOrdersCombinations:
    ...
    def test_basic_combinations(self):
        ...
        combos = get_paradoxical_orders_combinations(
            df, "A", elements_in_combinations=2
        )
    ...
    def test_larger_combinations(self):
        ...
        combos = get_paradoxical_orders_combinations(
            df, "A", elements_in_combinations=3
        )
    ...
    def test_single_element_combination(self):
        ...
        combos = get_paradoxical_orders_combinations(
            df, "A", elements_in_combinations=1
        )
...
class TestGetNewParadoxicalOrdersListAddingAndRemoving:
    ...
    def test_basic_new_combination(self):
        ...
        leftout = pd.DataFrame(
            {cols.FLOAT_RATIO_NET_INCOME_BID_ENERGY: [10, 20]},
            index=["9706994_SCO", "9707198_SCO"],
        )
        cleared = pd.DataFrame(
            {cols.FLOAT_RATIO_NET_INCOME_CLEARED_ENERGY: [5]},
            index=["9708010_SCO"],
        )
    ...
    def test_removes_cleared_groups(self):
        ...
        leftout = pd.DataFrame(
            {cols.FLOAT_RATIO_NET_INCOME_BID_ENERGY: [30]},  # High merit
            index=["9706994_SCO"],
        )
        cleared = pd.DataFrame(
            {cols.FLOAT_RATIO_NET_INCOME_CLEARED_ENERGY: [5]},  # Low (negative) merit
            index=["9708010_SCO"],
        )
        iterations_df = pd.DataFrame({cols.PARADOXICAL_ORDERS_COLUMN: []})
        starting = transform_ids_paradoxical_orders_list_to_dict(["9708010_SCO"])
    ...
    def test_empty_inputs(self):
        ...
        empty_leftout = pd.DataFrame({cols.FLOAT_RATIO_NET_INCOME_BID_ENERGY: []})
        empty_cleared = pd.DataFrame({cols.FLOAT_RATIO_NET_INCOME_CLEARED_ENERGY: []})
        iterations_df = pd.DataFrame({cols.PARADOXICAL_ORDERS_COLUMN: []})
    ...
    def test_respects_max_combinations_count(self):
        ...
        leftout = pd.DataFrame(
            {cols.FLOAT_RATIO_NET_INCOME_BID_ENERGY: [10, 20, 30, 40, 50]},
    ...
    def test_mixed_mic_scos_and_bid_blocks(self):
        ...
        leftout = pd.DataFrame(
            {cols.FLOAT_RATIO_NET_INCOME_BID_ENERGY: [10, 20, 30]},
            index=["9706994_SCO", "9707582_B_2_GE_0", "9707198_SCO"],
        )
        cleared = pd.DataFrame(
            {cols.FLOAT_RATIO_NET_INCOME_CLEARED_ENERGY: [5, 8]},
    ...
    def test_multiple_swaps_in_combination(self):
        ...
        leftout = pd.DataFrame(
            {cols.FLOAT_RATIO_NET_INCOME_BID_ENERGY: [30, 40]},
            index=["L1_SCO", "L2_SCO"],
        )
        cleared = pd.DataFrame(
            {cols.FLOAT_RATIO_NET_INCOME_CLEARED_ENERGY: [5, 6]},
            index=["C1_SCO", "C2_SCO"],
        )
        iterations_df = pd.DataFrame({cols.PARADOXICAL_ORDERS_COLUMN: []})
