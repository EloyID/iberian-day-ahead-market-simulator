...
def det_cab_verifications(det_raw, cab_raw):
    ...
    det_cab = det_cab.query(f"{INT_PERIODO} != 25")

    det_cab["cod_ofertada_casada"] = "O"
    det_cab[FLOAT_BID_POWER_CUMSUM] = get_float_bid_power_cumsum(
        det_cab,
    ...
    det_cab[ID_INDIVIDUAL_BID] = (
        det_cab[unique_identifiers].astype(str).agg("_".join, axis=1)
    )
    assert (
        not det_cab[ID_INDIVIDUAL_BID].duplicated().any()
    ...
    det_cab_aux = (
        det_cab.query(f"{INT_NUM_BLOQ} > 0")
        .groupby(bloq_unique_identifiers, observed=True)[FLOAT_BID_PRICE]
        .diff()
        .fillna(0)
    )
    assert det_cab_aux[
    ...
    det_cab_aux = (
        det_cab.query(f"{INT_NUM_BLOQ} > 0")
        .groupby(bloq_unique_identifiers, observed=True)[FLOAT_MAR]
        .diff()
        .fillna(0)
    )
    assert det_cab_aux[
    ...
    det_cab_potencia_offered_in_excess = (
        ...
        .agg({FLOAT_BID_POWER: "sum", FLOAT_MAX_POWER: "first", INT_NUM_BLOQ: "first"})
        .reset_index()
        .eval(f"{FLOAT_BID_POWER} = {FLOAT_BID_POWER}.round(2)")
        .query("{FLOAT_BID_POWER} > MaxPot")
    )
    ...
    for num_bloq in det_cab_not_compliant[INT_NUM_BLOQ].unique():
        det_cab_not_compliant_exclusive_option = det_cab_not_compliant.query(
            f"({INT_NUM_GRUPO_EXCL} > 0 and {INT_NUM_BLOQ} == @num_bloq) or ({INT_NUM_GRUPO_EXCL} == 0)"
        )
        det_cab_exclusive_option_potencia_offered_in_excess = (
            ...
            .agg(
                {
                    FLOAT_BID_POWER: "sum",
                    FLOAT_MAX_POWER: "first",
                    INT_NUM_BLOQ: "first",
                }
            )
            .reset_index()
            .eval(f"{FLOAT_BID_POWER} = {FLOAT_BID_POWER}.round(2)")
            .query(f"{FLOAT_BID_POWER} > {FLOAT_MAX_POWER}")
        )

        assert (
            det_cab_exclusive_option_potencia_offered_in_excess.empty
        ), f"There are offers with {INT_NUM_BLOQ} == {num_bloq} that cannot be accepted together due to MaxPot"

    cab_aux = cab[cab.duplicated(subset=[ID_UNIDAD, "dat_sesion"], keep=False)]
    assert len(cab_aux) == 0, "Some Unidades have more than one offer per day"

    assert det_cab.query(
        f"{INT_NUM_GRUPO_EXCL} > 0 and {INT_NUM_BLOQ} == 0"
    ).empty, "An exclusive offer must be a block offer"
    assert det_cab.query(
        f"{INT_NUM_BLOQ} == 0 and {FLOAT_MAR} > 0"
    ).empty, f"{FLOAT_MAR} > 0 only for block offers"
    assert det_cab.query(
        f"{INT_NUM_BLOQ} > 0 and {FLOAT_MAV} > 0"
    ).empty, f"{FLOAT_MAV} > 0 only for SCO or normal offers"
    assert det_cab.query(
        f"{INT_NUM_BLOQ} > 0 and {FLOAT_MIC} > 0"
    ).empty, f"{FLOAT_MIC} > 0 only for SCO offers"
    assert det_cab.query(
        f"{INT_NUM_BLOQ} > 0 and {INT_NUM_TRAMO} > 1"
    ).empty, f"{INT_NUM_TRAMO} > 1 only for SCO and simple offers"
    assert det_cab.query(f"{FLOAT_MIC} >= 0").empty, f"{FLOAT_MIC} must be >= 0"

    det_cab_C = det_cab.query(f"{CAT_BUY_SELL} == 'C'").copy()
    det_cab_V = det_cab.query(f"{CAT_BUY_SELL} == 'V'").copy()
    ...
    det_cab_V_aux = (
        det_cab_V.copy()
        .query(f"{INT_NUM_BLOQ} == 0")
        .sort_values(["dat_sesion", ID_ORDER, INT_PERIODO, INT_NUM_TRAMO])
    )
    det_cab_V_aux["Diff Tramo PrecEuro"] = (
        det_cab_V_aux.groupby(["dat_sesion", ID_ORDER, INT_PERIODO])[FLOAT_BID_PRICE]
        .diff()
        .fillna(0)
    )
    ...
    det_cab_C_aux["Diff Tramo PrecEuro"] = (
        det_cab_C_aux.groupby(["dat_sesion", ID_ORDER, INT_PERIODO])[FLOAT_BID_PRICE]
        .diff()
        .fillna(0)
    )
    ...
    det_cab_aux["NumTramo_diff"] = (
        det_cab_aux.groupby(["dat_sesion", ID_ORDER, INT_PERIODO])[INT_NUM_TRAMO]
        .diff()
        .fillna(1)
    )
    assert (
