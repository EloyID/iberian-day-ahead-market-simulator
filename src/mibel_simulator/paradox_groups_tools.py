...
def transform_ids_paradox_groups_list_to_dict(
    ...
    """
    Transforms a list of paradox order IDs into a dictionary separating MIC SCOs, bid blocks, and complex orders.

    Args:
        ids_paradox_groups (list): List of paradox order IDs.
    Returns:
        dict: Dictionary with keys 'ids_mic_scos', 'ids_bid_blocks', and 'ids_complex_orders' containing respective IDs.
    """
    ...
    ids_complex_orders = [id for id in ids_paradox_groups if "COMPLEX_ORDER" in str(id)]

    assert len(ids_mic_scos) + len(ids_bid_blocks) + len(ids_complex_orders) == len(
        ids_paradox_groups
    ), (
        "Error in transforming paradox groups list to dict: "
        "some IDs do not match MIC SCOs, bid blocks, or complex orders."
    )
...
def transform_paradox_groups_dict_to_ids_list(
    ...
    Args:
        paradox_groups_dict (dict): Dictionary with keys 'ids_mic_scos', 'ids_bid_blocks', and 'ids_complex_orders' containing respective IDs.
    Returns:
    ...
    ids_complex_orders = paradox_groups_dict[cols.IDS_COMPLEX_ORDERS]
    return ids_mic_scos + ids_bid_blocks + ids_complex_orders
