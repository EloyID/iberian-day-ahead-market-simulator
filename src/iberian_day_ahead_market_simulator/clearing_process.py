...
def get_leftout_paradoxical_orders_summary(
    ...
    Args:
        ...
        all_paradoxical_orders (dict): Dictionary of all paradox order IDs.
        iteration_paradoxical_orders (dict): Dictionary of paradox order IDs included in the iteration.
        clearing_price_df (pd.DataFrame): DataFrame of clearing prices per period and zone.
...
def get_new_paradoxical_orders_list_by_removing_underperforming_ones(
    iteration_cleared_paradoxical_orders_summary: pd.DataFrame,
    iterations_df: pd.DataFrame,
    paradoxical_orders_combination: dict,
    ...
    and checks if the resulting combinations have already been tested. Returns up to int_paradoxical_orders_count new combinations that have not been tested yet.

    Args:
        iteration_cleared_paradoxical_orders_summary (pd.DataFrame): Summary DataFrame of cleared paradox orders for the current iteration.
        iterations_df (pd.DataFrame): DataFrame of all previous iteration results.
        paradoxical_orders_combination (dict): Dictionary of paradox orders in the current iteration.
        int_paradoxical_orders_count (int, optional): Maximum number of new combinations to return. Defaults to 1.
    ...
    new_paradoxical_orders_df = pd.DataFrame(
        {
            cols.PARADOXICAL_ORDERS_COLUMN: np.nan,
            cols.BOOL_ARE_PARADOXICAL_ORDERS_TESTED: np.nan,
        },
        index=iteration_cleared_paradoxical_orders_summary.index,
    ).astype(
        {
            cols.PARADOXICAL_ORDERS_COLUMN: object,
            cols.BOOL_ARE_PARADOXICAL_ORDERS_TESTED: bool,
        }
    )

    paradoxical_order_ids_left = transform_paradoxical_orders_dict_to_ids_list(
        paradoxical_orders_combination
    )

    for index, row in new_paradoxical_orders_df.iterrows():
        new_iteration_paradoxical_orders = list(
            set(paradoxical_order_ids_left) - set([index])
        )
        are_paradoxical_orders_tested = check_are_paradoxical_orders_tested(
            iterations_df, new_iteration_paradoxical_orders
        )

        new_paradoxical_orders_df.at[index, cols.PARADOXICAL_ORDERS_COLUMN] = (
            new_iteration_paradoxical_orders
        )
        new_paradoxical_orders_df.at[index, cols.BOOL_ARE_PARADOXICAL_ORDERS_TESTED] = (
            are_paradoxical_orders_tested
        )

        paradoxical_order_ids_left = new_iteration_paradoxical_orders

    new_paradoxical_orders_df = new_paradoxical_orders_df.query(
        f"{cols.BOOL_ARE_PARADOXICAL_ORDERS_TESTED} == False"
    ).head(int_paradoxical_orders_count)

    return (
        new_paradoxical_orders_df[cols.PARADOXICAL_ORDERS_COLUMN]
        .apply(transform_ids_paradoxical_orders_list_to_dict)
        .tolist()
    )


def define_new_paradoxical_orders_list(
    iterations_df: pd.DataFrame,
    det_cab: pd.DataFrame,
    all_paradoxical_orders: dict,
    is_QH: bool,
    ...
    Args:
        ...
        det_cab (pd.DataFrame): Full DET/CAB DataFrame.
        all_paradoxical_orders (dict): Dictionary of all paradox groups with MIC.
        int_paradoxical_orders_count (int, optional): Maximum number of MIC paradox group combinations to propose. Defaults to 1.
    ...
    int_paradoxical_orders_count_start = int_paradoxical_orders_count
    new_paradoxical_orders_list = []
    for index, row in sorted_promising_iterations_df.iterrows():
        ...
        if is_expected_income_respected:
            ...
            new_paradoxical_orders_list.extend(
                get_new_paradoxical_orders_list_adding_and_removing(
                    leftout_paradoxical_orders_summary,
                    iteration_cleared_paradoxical_orders_summary,
                    iterations_df,
                    paradoxical_orders,
                    int_paradoxical_orders_count,
                )
            )
            ...
            new_paradoxical_orders_list.extend(
                get_new_paradoxical_orders_list_by_removing_underperforming_ones(
                    iteration_cleared_paradoxical_orders_summary,
                    iterations_df,
                    paradoxical_orders,
                    int_paradoxical_orders_count,
        ...
        int_paradoxical_orders_count = int_paradoxical_orders_count_start - len(
            new_paradoxical_orders_list
        )
        if int_paradoxical_orders_count <= 0:
            break

    return new_paradoxical_orders_list
...
def iterative_function(
    ...
    bool_is_expected_income_respected = (
        cleared_paradoxical_orders_summary[cols.FLOAT_NET_INCOME] >= 0
    ).all()

    ids_mic_scos = paradoxical_orders[cols.IDS_MIC_SCOS]
    ids_bid_blocks = paradoxical_orders[cols.IDS_BID_BLOCKS]

    # Update iterations_df with current iteration results
    iteration_df_entry = {
        cols.PARADOXICAL_ORDERS_COLUMN: [paradoxical_orders],
        cols.IDS_MIC_SCOS: [ids_mic_scos],
        cols.IDS_BID_BLOCKS: [ids_bid_blocks],
        cols.IDS_PARADOXICAL_ORDERS: [ids_mic_scos + ids_bid_blocks],
...
def check_if_success_at_first_iteration(
    ...
    is_iteration_paradoxical_orders_provided: bool,
) -> bool:
    """
    ...
    Args:
        ...
        is_iteration_paradoxical_orders_provided (bool): Indicates if an initial paradox orders combination was provided.
    ...
    success_at_first_iteration = (
        ...
        and not is_iteration_paradoxical_orders_provided
...
def run_iterative_loop(
    ...
    are_iteration_ids_paradoxical_orders_provided = (
        is_iteration_ids_mic_scos_provided and is_iteration_ids_bid_blocks_provided
    )

    all_paradoxical_orders = get_all_paradoxical_orders(det_cab)
    ...
    # If iteration_paradoxical_orders is provided, start with that combination
    ...
    # Otherwise start with all paradox orders
        ...
        next_iterations_paradoxical_orders = [all_paradoxical_orders]
    ...
    while completed_iterations < iterations_count:
        ...
        args_list = [
            ...
            for iteration_paradoxical_orders in next_iterations_paradoxical_orders
        ]
        with multiprocessing.Pool(processes=n_jobs) as pool:
            ...
            ### hasta aqui
        new_iteration_int_paradoxical_orders_count = min(
        ...
        if len(next_iterations_paradoxical_orders) == 0:
            break
    ...
    det_cab_scos_filtered = filter_paradoxical_orders_from_det_cab(
        det_cab, best_iteration[cols.PARADOXICAL_ORDERS_COLUMN]
    )
    ...
    # Attach the solver results for the winning combination only.
    # Pyomo's SolverResults is iterable/mapping-like. Pre-creating the
    # column (if needed) and using `.at` for the scalar set avoids that
    # broadcasting path entirely.
    if cols.SOLVER_RESULTS_COLUMN not in iterations_df.columns:
        iterations_df[cols.SOLVER_RESULTS_COLUMN] = None
    iterations_df.at[best_iteration.name, cols.SOLVER_RESULTS_COLUMN] = results
